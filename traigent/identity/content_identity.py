...
:func:`derive_tenant_keys` is here only because the conformance vectors pin
the derivation; production code receives the two derived purpose keys and
``kid`` from the Backend (:mod:`traigent.identity.keys`).
...
def _check_keys(keys: TenantIdentityKeys) -> None:
    if not isinstance(keys, TenantIdentityKeys) or not _KEY_ID_RE.fullmatch(
        ...
        raise ContentIdentityError(
            "keys must be TenantIdentityKeys from derive_tenant_keys"
        )
