...
#: Canonical vocabulary for "this 403/failure came from the network EDGE, not from
#: Traigent". Defined once and imported by `traigent.cli.auth_commands`, which used
#: to keep a hand-synced copy: two lists meant `whoami` and a session-creation
#: failure could classify the same backend response differently, which is the exact
#: defect #1754 fixed on this path and #1775 reported on the CLI.
EDGE_BLOCK_SIGNALS: tuple[str, ...] = (
    "edge_blocked",
    "edge blocked",
    "cloudflare",
    "cf-ray",
    ...
    "x-amzn-waf",
    # Akamai reference-id block pages
    "akamai",
    "reference #",
    "waf",
)
...
def _classify_session_creation_failure(
    ...
    *,
    ...
    if _contains_any(text, EDGE_BLOCK_SIGNALS):
