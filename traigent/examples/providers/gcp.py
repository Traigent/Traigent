"""Quickstart example — Google Cloud Vertex AI via LiteLLM.

LiteLLM is a core Traigent dependency, so this runs with no extra
packages.

Vertex AI needs a GCP project, a location, and Application Default
Credentials (not just an API key):

    VERTEXAI_PROJECT=my-project  VERTEXAI_LOCATION=us-central1
    GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json   # or: gcloud auth application-default login

    # mock (no key, no spend):
    python -m traigent.examples.providers.gcp
    # real:
    TRAIGENT_MOCK_LLM=false python -m traigent.examples.providers.gcp
"""
