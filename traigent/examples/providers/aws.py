"""Quickstart example — AWS Bedrock via LiteLLM.

LiteLLM is a core Traigent dependency, so this runs with no extra
packages.

Bedrock needs AWS credentials and a region; model ids are
region/account dependent and require model access to be enabled:

    AWS_ACCESS_KEY_ID=...  AWS_SECRET_ACCESS_KEY=...  AWS_REGION_NAME=us-east-1

    # mock (no key, no spend):
    python -m traigent.examples.providers.aws
    # real:
    TRAIGENT_MOCK_LLM=false python -m traigent.examples.providers.aws
"""
