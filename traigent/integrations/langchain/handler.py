...
class TraigentHandler(BaseCallbackHandler):
    ...
    # NOTE: There is deliberately no on_chat_model_end / on_chat_model_error
    # here. LangChain's BaseCallbackHandler defines on_chat_model_START but has
    # no *_end / *_error chat-model hooks — chat-model runs finalize through
    # on_llm_end / on_llm_error (below), which is what actually captures the
    # metrics. Prior versions defined those two methods with docstrings
    # claiming the inverse; they were dead no-ops LangChain never invoked and
    # were removed (see issue #1973).
