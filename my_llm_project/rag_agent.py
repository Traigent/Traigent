...
Usage (run from repo root):
    ...
    .venv/bin/python my_llm_project/rag_agent.py --model gpt-5.2 --temperature 0.0 --max-tokens 50 --instructions CoT
"""
...
def main() -> None:
    ...
    for i, ex in enumerate(examples, 1):
        ...
        print(f"\rEvaluating: [{bar}] {i}/{total}  Accuracy: {accuracy:.1%}  ", end="", flush=True)

    print(f"\n\nFinal Accuracy: {sum(scores) / total:.1%}")
