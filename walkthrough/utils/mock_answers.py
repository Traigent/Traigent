...
RAG_ANSWERS = {
    ...
    "what execution modes does traigent support?": "Local, cloud, and hybrid_api — local runs and picks configurations on your machine, cloud runs on your machine but lets Traigent's backend pick configurations, and hybrid_api calls your agent as an external service. edge_analytics has been removed",
    "how does local mode work?": "Local mode keeps all data on your machine for complete privacy",
    "what is edge_analytics mode?": "edge_analytics mode has been removed. Use local mode (offline=True) for on-device optimization with zero egress",
    ...
    "explain cloud mode with privacy": "Cloud mode keeps your agent's execution on your machine; only run metrics are sent to Traigent's backend for optimization, not raw prompts or outputs",
    ...
    "what privacy features does traigent offer?": "Local storage and offline=True for zero-egress runs; the privacy_enabled flag and edge_analytics mode are deprecated",
