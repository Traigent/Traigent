...
class FakeResult:
    ...
    def calculate_weighted_scores(self, **_) -> dict[str, dict[str, Any]]:
...
def progress_animation() -> None:
    ...
    for i in range(1, total + 1):
        ...
        best_so_far = max(trial[5] for trial in TRIALS_DATA[:i])
        print(
            f"\rOptimizing: [{bar}] {i}/{total}  Best accuracy: {best_so_far:.1%}  ",
            end="",
            flush=True,
        )
        time.sleep(0.4)
