...
class SideEffectFramework:
    """Framework whose instantiation should be skipped by default."""
...
class TestParameterDiscoveryInit:
    ...
    def test_discover_private_parameters(self, discovery):
        ...
        # Private parameters are name-mangled in Python
        assert "_MockPrivateClass__private_param" in params
...
class TestFrameworkIntrospection:
    ...
    def test_find_completion_methods_skips_instantiation_by_default(self, discovery):
        """Ensure discovery does not instantiate classes without opt-in flag."""
        SideEffectFramework.init_calls = 0

        methods = discovery.find_completion_methods(SideEffectFramework)

        assert isinstance(methods, list)
        assert SideEffectFramework.init_calls == 0
