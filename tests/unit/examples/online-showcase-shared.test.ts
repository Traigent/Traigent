...
describe('online showcase shared helpers', () => {
  ...
  it('keeps partial helper evidence when session listing fails', async () => {
    mocks.listOptimizationSessions.mockRejectedValueOnce(new Error('listing unavailable'));
    ...
    expect(helpers.helperErrors).toEqual([{ helper: 'listed', message: 'listing unavailable' }]);
