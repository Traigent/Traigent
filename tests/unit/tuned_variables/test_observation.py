...
def test_full_mode_emits_free_text_verbatim(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("TRAIGENT_TVAR_OBSERVATION", "full")
    free_text = "free text with spaces is allowed in full mode"

    observation = build_tvar_observation(
        session_id="session-5",
        trial_id="trial-5",
        config={"free_knob": free_text},
        metrics={"answer_em": 0.6},
        primary_metric="answer_em",
        comparability={"scope": "trial", "n": 3},
        sdk_version="9.9.9",
    )

    assert observation is not None
    assert observation["privacy"] == {"raw_content_included": False}
    assert observation["variables"] == [{"name": "free_knob", "value": free_text}]
