"""Unit tests for tokencost integration in evaluators."""
...
class TestTokencostIntegration:
    """Test tokencost integration in metrics extraction."""

    def test_extract_llm_metrics_with_tokencost_available(self):
        """Test extract_llm_metrics when tokencost is available."""
    ...
    def test_extract_llm_metrics_without_tokencost(self):
        """Test extract_llm_metrics when tokencost is not available."""
        response = MockOpenAIResponse("positive", 15, 8)
        model_name = "gpt-4o-mini"

        # Mock tokencost not available - mock the CostCalculator to not calculate any cost
        def mock_calculate_cost(
            self,
            metrics,
            model_name,
        ...
        with (
            ...
            # Verify cost remains zero without tokencost
    ...
    def test_extract_llm_metrics_with_existing_cost_in_response(self):
        ...
        # Mock tokencost not available to test fallback - mock CostCalculator to not override costs
        def mock_calculate_cost(
            self,
            metrics,
            model_name,
    ...
    def test_extract_llm_metrics_handles_tokencost_exceptions(self):
        """Test graceful handling of tokencost exceptions."""
        response = MockOpenAIResponse("positive", 15, 8)
        model_name = "gpt-4o-mini"
...
class TestLocalEvaluatorWithTokencost:
    """Test LocalEvaluator integration with tokencost."""
    ...
    @pytest.mark.asyncio
    async def test_local_evaluator_with_tokencost_integration(
        self, sample_dataset, mock_function
    ):
        """Test that LocalEvaluator properly integrates with tokencost."""
    ...
    @pytest.mark.asyncio
    async def test_local_evaluator_fallback_without_tokencost(
        self, sample_dataset, mock_function
    ):
        """Test that LocalEvaluator works without tokencost."""
        evaluator = LocalEvaluator(metrics=["accuracy"])
        config = {"model": "gpt-4o-mini", "temperature": 0.0}
        ...
        with patch("traigent.evaluators.metrics_tracker.TOKENCOST_AVAILABLE", False):
            ...
            # Cost will be zero without tokencost, but shouldn't break
            assert "cost" in result.aggregated_metrics
...
class TestCostTrackingEdgeCases:
    ...
    def test_missing_model_name(self):
        ...
        with patch("traigent.evaluators.metrics_tracker.TOKENCOST_AVAILABLE", True):
            ...
            # Without model name, tokencost shouldn't be called
            assert metrics.cost.total_cost == 0.0
...
async def test_cost_from_token_counts_anthropic_alias_fields(monkeypatch):
    """Ensure cost uses token-count fallback for Anthropic alias fields via tokencost per-token rates."""
    from traigent.evaluators.metrics_tracker import extract_llm_metrics

    # Use actual tokencost pricing if available, otherwise use test values
    try:
        import tokencost

        if "claude-3-haiku-20240307" in tokencost.TOKEN_COSTS:
            # Use actual pricing from tokencost
            actual_pricing = tokencost.TOKEN_COSTS["claude-3-haiku-20240307"]
            ...
            # Use test values if model not in tokencost
            input_rate = 0.00000025
            output_rate = 0.00000075
    except ImportError:
        # Use test values if tokencost not available
        input_rate = 0.00000025
        output_rate = 0.00000075
...
def test_cost_from_token_counts_openai(monkeypatch):
    """Ensure cost uses token-count fallback for OpenAI responses with tokencost per-token rates."""
    from traigent.evaluators.metrics_tracker import extract_llm_metrics
    ...
    fake_tokencost = types.SimpleNamespace()
    ...
    monkeypatch.setitem(__import__("sys").modules, "tokencost", fake_tokencost)
    monkeypatch.setattr(
...
def test_fallback_cost_from_prompt_response_when_no_tokens(monkeypatch):
    """If no tokens available, ensure prompt/response path computes cost via tokencost functions."""
    from traigent.evaluators.metrics_tracker import extract_llm_metrics
