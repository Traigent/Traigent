...
class TestExperimentStats:
    """Tests for experiment-level statistics computed on OptimizationResult."""
    ...
    def _make_trial(
        ...
        *,
        ...
        """Helper to build TrialResult instances concisely."""
    ...
    def test_experiment_stats_basic_aggregation(self):
        """Experiment stats aggregate duration, cost, configs, and statuses."""
        ...
        trials = [
            self._make_trial(
                ...
                config={"model": "a", "temperature": 0.1},
            ),
            self._make_trial(
                "t2",
                status=TrialStatus.FAILED,
                duration=1.0,
                metadata={"cost": {"total_cost": 0.2}, "failed": True},
                config={"model": "a", "temperature": 0.1},
                error_message="boom",
            ),
            self._make_trial(
                ...
                status=TrialStatus.CANCELLED,
                duration=0.5,
                config={"model": "b", "temperature": 0.1},
            ),
        ]
        ...
        # Cached property should reuse the same object
        assert stats is result.experiment_stats

        assert stats.total_duration == pytest.approx(10.0)
        assert stats.total_cost == pytest.approx(0.7)
        assert stats.unique_configurations == 2
        assert stats.trial_counts["total"] == 3
        assert stats.trial_counts["completed"] == 1
        assert stats.trial_counts["failed"] == 1
        assert stats.trial_counts["cancelled"] == 1
        assert stats.trial_counts["exceptions"] == 1
        assert stats.average_trial_duration == pytest.approx((2.0 + 1.0 + 0.5) / 3)
        assert stats.cost_per_configuration == pytest.approx(0.35)
        assert stats.success_rate == pytest.approx(1 / 3)
        assert stats.error_message is None

    def test_experiment_stats_respects_top_level_cost(self):
        """Top-level total_cost overrides per-trial cost aggregation."""
        ...
        trials = [
            self._make_trial(
                ...
                status=TrialStatus.COMPLETED,
                duration=1.0,
                metrics={"total_cost": 0.1},
            ),
            self._make_trial(
                "t2",
                status=TrialStatus.FAILED,
                duration=1.5,
                metrics={"total_cost": 0.3},
            ),
        ]
        ...
        stats = result.experiment_stats
        assert stats.total_cost == pytest.approx(5.5)
        # Duration falls back to per-trial sum when top-level is missing/zero
        assert stats.total_duration == pytest.approx(2.5)
        assert stats.average_trial_duration == pytest.approx(1.25)

    def test_experiment_stats_handles_malformed_cost(self):
        """Malformed cost entries should not break stats computation."""
        ...
        assert stats.trial_counts["total"] == 1
        assert stats.trial_counts["failed"] == 1
        assert stats.total_duration == pytest.approx(0.0)
