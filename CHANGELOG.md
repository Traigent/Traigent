...
- **The run's finalize request now carries a selection receipt.** When the SDK picks a
  winner, the `session_aggregation` sent on session finalize includes `selection`: the
  winning trial id, the exact ranking-eligible trial ids it was chosen over (unique,
  sorted), their count and a `sha256:` digest, the SDK's selection reason code, and the
  ...
  winner means no `selection` key; a margin with a non-finite number is omitted rather
