...
\begin{enumerate}[nosep]
  \item \textbf{Discover} --- Traigent calls \texttt{GET /capabilities} and
        \texttt{GET /config-space} to learn which parameters are tunable, their
        domains, and declared objectives.
  \item \textbf{Execute} --- For each trial, Traigent posts a candidate
        configuration and a batch of inputs to \texttt{POST /execute}. Your
  ...
  \item \textbf{Evaluate} --- If a separate evaluation endpoint is available,
        Traigent posts output--target pairs to \texttt{POST /evaluate}. Your
...
\midrule
Discover     & Traigent $\to$ Service & \texttt{GET /capabilities}, \texttt{GET /config-space} \\
Execute      & Traigent $\to$ Service & \texttt{POST /execute} \\
Evaluate     & Traigent $\to$ Service & \texttt{POST /evaluate} \\
...
\texttt{Authorization} header (e.g.\ bearer token). Configure the Traigent SDK
client with:
...
\field{supports\_evaluate}   & boolean & \optional & Separate \texttt{/evaluate} endpoint available (default: \texttt{true}) \\
...
\field{constraints}      & object & \optional & Structural / behavioral constraints \\
...
\field{execution\_id}        & string & \required & Unique execution ID (referenced by \texttt{/evaluate}) \\
...
\field{evaluations}    & array         & \optional & Output--target pairs \\
...
\midrule
200 & Success \\
400 & Bad Request (invalid input, capability mismatch) \\
401 & Unauthorized \\
404 & Not Found (unknown route or session) \\
...
All \texttt{POST} requests accept a \field{request\_id} field (UUID). If the
same \field{request\_id} is received again, the service may return the cached
response. This enables safe retries without duplicate processing.
