...
fun addFloat[a, b: Float]: Float {
    { f: Float | f.value = a.value + b.value }
}
...
-- Invariant I4: accumulated + reserved <= limit
...
pred acquirePermit[e: CostEnforcer, t: Time, tNext: Time, p: Permit, estimated: Float] {
    -- Precondition: budget available
    let projected = addFloat[addFloat[e.accumulated_cost[t], e.reserved_cost[t]], estimated] |
        leq[projected, e.limit]

    -- Postcondition: state updated atomically
    e.permit_counter[tNext] = e.permit_counter[t] + 1
    p.id = e.permit_counter[tNext]
    p.amount = estimated
    p.active[tNext] = True
    e.reserved_cost[tNext].value = e.reserved_cost[t].value + estimated.value
    e.in_flight_count[tNext] = e.in_flight_count[t] + 1
    e.active_permits[tNext] = e.active_permits[t] + p
    e.accumulated_cost[tNext] = e.accumulated_cost[t]
}

-- Deny permit (budget exceeded)
pred denyPermit[e: CostEnforcer, t: Time, tNext: Time, p: Permit] {
...
pred releasePermit[e: CostEnforcer, t: Time, tNext: Time, p: Permit] {
    ...
    e.active_permits[tNext] = e.active_permits[t] - p
    e.in_flight_count[tNext] = e.in_flight_count[t] - 1
    e.reserved_cost[tNext].value = e.reserved_cost[t].value - p.amount.value
    e.accumulated_cost[tNext] = e.accumulated_cost[t]
    e.permit_counter[tNext] = e.permit_counter[t]
}
...
pred trackCost[e: CostEnforcer, t: Time, tNext: Time, p: Permit, actual: Float] {
    ...
    e.in_flight_count[tNext] = e.in_flight_count[t] - 1
    e.reserved_cost[tNext].value = e.reserved_cost[t].value - p.amount.value
    e.accumulated_cost[tNext].value = e.accumulated_cost[t].value + actual.value
    e.permit_counter[tNext] = e.permit_counter[t]
}
...
pred frame[e: CostEnforcer, t: Time, tNext: Time] {
    ...
    (some p: Permit | denyPermit[e, t, tNext, p]) or
...
-- Assert: No budget violation is reachable from valid initial state
assert NoBudgetViolation {
    all e: CostEnforcer | validTrace[e] implies {
        all t: Time | I4_BudgetBound[e, t]
    }
}
...
check NoBudgetViolation for 5 but 10 Time, 10 Permit, 10 Float
...
-- Find a valid trace with 5 permits acquired
run showValidTrace {
    some e: CostEnforcer | validTrace[e] and #Permit >= 5
} for 5 but 10 Time, 10 Permit, 10 Float

-- Find a trace where budget limit is hit
run showBudgetExhaustion {
    some e: CostEnforcer, t: Time |
        validTrace[e] and
        e.accumulated_cost[t].value + e.reserved_cost[t].value >= e.limit.value
} for 5 but 10 Time, 10 Permit, 10 Float

-- Find a double-release scenario
run showDoubleRelease {
    some e: CostEnforcer, p: Permit, t: Time, tNext: Time |
        validTrace[e] and doubleRelease[e, t, tNext, p]
