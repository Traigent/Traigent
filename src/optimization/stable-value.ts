...
function canonicalizeValue(value: unknown, seen: WeakSet<object>, path: string): unknown {
  ...
  if (typeof value === 'function' || typeof value === 'symbol') {
    throw new ValidationError(`Unsupported ${typeof value} in optimization value at ${path}.`);
  }

  if (Array.isArray(value)) {
    if (seen.has(value)) {
      throw new ValidationError(`Circular optimization values are not supported at ${path}.`);
    }
    seen.add(value);
  ...
  if (!isPlainObject(value)) {
    throw new ValidationError(`Unsupported object value in optimization metadata at ${path}.`);
  }
