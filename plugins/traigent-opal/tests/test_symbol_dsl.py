...
def test_builder_optimize_forwards_full_traigent_options(
    ...
    with opal_program(module="examples.options") as v:
        v.model in tv(choices("a", "b"))
        v.maximize("quality")
    ...
    @v.optimize(
        ...
        budget_limit=1.8,
        budget_metric="cost",
        budget_include_pruned=False,
        max_total_examples=64,
    )
    def run() -> str:
        return v.model
    ...
    assert run() == "a"
    assert captured["model"] == ["a", "b"]
    assert captured["algorithm"] == "tpe"
    assert captured["max_trials"] == 30
    ...
    assert captured["budget_limit"] == 1.8
    assert captured["budget_metric"] == "cost"
    assert captured["budget_include_pruned"] is False
    assert captured["max_total_examples"] == 64


def test_builder_optimize_supports_async_functions(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def fake_optimize(**_: Any):
        def _decorator(fn):
            return fn
    ...
    with opal_program(module="examples.async") as v:
        v.model in tv(choices("a", "b"))
        v.maximize("quality")

    @v.optimize(execution={"runtime": "python"})
