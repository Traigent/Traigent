...
INPUT_IDS: Final[list[str]] = [x.strip() for x in _INPUT_IDS_RAW.split(",") if x.strip()]
...
def serialize_result(result: OptimizationResult) -> dict[str, Any]:
    ...
    for t in result.trials:
        trials.append({
            "trial_id": t.trial_id,
            "config": t.config,
            "metrics": {k: v for k, v in t.metrics.items() if isinstance(v, (int, float, type(None)))},
...
def write_report(data: dict[str, Any], path: Path) -> None:
    ...
    for i, t in enumerate(data["trials"]):
        ...
        lines.append(f"| {i+1} | {model} | {ta_str} | {pa_str} | {xa_str} | {t['status']} |")
...
async def run_optimization() -> None:
    ...
    async with evaluator:
        ...
        print(f"      Grid size: {grid_size} configs, max_trials: {max_trials}, examples: {len(INPUT_IDS)} ({max_trials * len(INPUT_IDS)} evaluations)")
