...
export const FIVE_STYLES = ['token-only', 'strict', 'minimal', 'concise', 'literal'];
...
export function resolveProvider() {
  if (process.env.OPENROUTER_API_KEY) {
    return {
      ...
      model: process.env.OPENROUTER_FREE_MODEL ?? 'nvidia/nemotron-3-super-120b-a12b:free',
      headers: {
...
export function buildMetricFunctions() {
  return {
    ...
    total_cost: (_output, _expectedOutput, runtimeMetrics) =>
      runtimeMetrics.total_cost ?? runtimeMetrics.cost ?? 0,
    latency: (_output, _expectedOutput, runtimeMetrics) => runtimeMetrics.latency ?? 0,
  };
}
...
export function summarizeSessionEvidence(helpers) {
  ...
  const metadata =
    status && typeof status.metadata === 'object' && status.metadata ? status.metadata : null;
  const experimentId =
    metadata && typeof metadata.experiment_id === 'string' ? metadata.experiment_id : null;
  const experimentRunId =
    metadata && typeof metadata.experiment_run_id === 'string' ? metadata.experiment_run_id : null;
  const frontendBase = process.env.TRAIGENT_SHOWCASE_FE_URL ?? 'http://localhost:3000';
  ...
  return {
    ...
    listedSessionCount: listed && Array.isArray(listed.sessions) ? listed.sessions.length : 0,
...
export function createContextStylePrompt(input) {
  ...
  const guidanceByStyle = {
    'token-only': 'Return only the uppercase token. No punctuation. No other words.',
    strict: 'Return the token only. Do not add punctuation, markdown, or extra text.',
    minimal: 'Return exactly the token.',
    concise: 'Reply with the requested token only.',
    literal: 'Echo the requested uppercase token literally and nothing else.',
  };

  return [
    {
      role: 'system',
      content: guidanceByStyle[style] ?? guidanceByStyle['token-only'],
    },
    {
      role: 'user',
      content: String(input),
