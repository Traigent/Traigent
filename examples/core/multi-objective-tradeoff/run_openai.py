#!/usr/bin/env python3
"""Math expression evaluation with multi-objective optimization (accuracy vs cost).
...
OBJECTIVE_SCHEMA = ObjectiveSchema.from_objectives(
    [
        ...
        ObjectiveDefinition(name="total_cost", orientation="minimize", weight=0.001),
    ]
)
...
@traigent.optimize(
    ...
    configuration_space={
        "model": [
            # "openai/invalid-model-xyz",  # Test invalid model
            "openai/gpt-4o-mini",
            "openai/gpt-4.1-nano",
...
def answer(
    ...
    # Remove "openai/" prefix if present for the ChatOpenAI constructor
