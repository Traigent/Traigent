...
class TestCustomAccuracyScorerPositive:
    ...
    def test_contains_at_end(self):
        assert custom_accuracy_scorer("The capital of France is Paris.", "Paris", {}) == 1.0

    def test_contains_at_start(self):
        assert custom_accuracy_scorer("Jupiter is the largest planet", "Jupiter", {}) == 1.0

    def test_contains_in_middle(self):
        assert custom_accuracy_scorer("The answer is 4, which is correct", "4", {}) == 1.0
    ...
    def test_multi_word_exact(self):
        assert custom_accuracy_scorer("Leonardo da Vinci", "Leonardo da Vinci", {}) == 1.0

    def test_multi_word_contained(self):
        assert custom_accuracy_scorer("The artist Leonardo da Vinci painted it", "Leonardo da Vinci", {}) == 1.0

    def test_with_trailing_punctuation(self):
        assert custom_accuracy_scorer("William Shakespeare.", "William Shakespeare", {}) == 1.0

    def test_with_surrounding_text_and_punctuation(self):
        assert custom_accuracy_scorer("The chemical symbol for water is H2O.", "H2O", {}) == 1.0
...
class TestCustomAccuracyScorerNegative:
    ...
    def test_reversed_containment(self):
        assert custom_accuracy_scorer("Paris", "The capital of France is Paris", {}) == 0.0

    def test_different_numbers(self):
        assert custom_accuracy_scorer("299,792 kilometers", "299,792,458 meters per second", {}) == 0.0

    def test_similar_but_not_contained(self):
        assert custom_accuracy_scorer("William Shakespear", "William Shakespeare", {}) == 0.0

    def test_substring_of_expected_not_full(self):
        assert custom_accuracy_scorer("Leonardo painted this", "Leonardo da Vinci", {}) == 0.0
...
class TestCustomAccuracyScorerEdgeCases:
    ...
    def test_llm_metrics_ignored(self):
        assert custom_accuracy_scorer("Paris", "Paris", {"tokens": 100, "latency": 0.5}) == 1.0
